    ...
    <nav>
      <ul className={" items-center md:gap-6 md:flex hidden"}>
        {defaultList.children.map(item => <li key={item.link}><Link
          className={twMerge(`font-merriweather font-semibold text-gray-900 text-sm ${pathName === item.link ? "text-red-700" : ""}`)}
          href={item.link}>{item.title}</Link></li>)}
        {(data?.user as any)?.role === "ADMIN" && <li key={"/admin"}><Link
          className={twMerge(`font-merriweather font-semibold text-gray-900 text-sm ${pathName === "/admin" ? "text-red-700" : ""}`)}
